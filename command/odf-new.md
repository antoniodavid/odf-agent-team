...
10. **Run DECIDE** through the compatible `PROPOSE` and `ASSESS` adapters only for non-micro routes. Micro routes materialize their terminal DECIDE through the bind in step 7.
11. **Run optional PLAN**, then BUILD and VERIFY according to the resolved route. Micro routes use their terminal prefix directly; do not reintroduce PROPOSE/ASSESS, QA-PLAN, or DESIGN.
12. If `--fast`, skip voluntary approval gates only where existing compatibility permits; never skip preflight, Policy Gate, validation evidence, a route-required VERIFY, or failure disposition.
