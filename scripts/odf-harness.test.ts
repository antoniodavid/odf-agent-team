...
describe("harness smoke: core determinism", () => {
  ...
  it("agent resolution: applies every phase, specialist routing, and bounded T8/T9/T10 routing", () => {
    ...
    expect(resolveAgent(registry, "ASSESS", ["migration", "version"])).toBe("odoo_upgrade_migrator")
    expect(resolveAgent(registry, "ASSESS", ["standard", "custom"])).toBe("odoo_functional_consultant")
    expect(resolveAgent(registry, "ASSESS", [])).toBe("odoo_functional_consultant")
    ...
    expect(resolveAgent(registry, "DESIGN", ["migration", "version"])).toBe("odoo_upgrade_migrator")
    expect(resolveAgent(registry, "IMPLEMENT", ["bounded", "batch", "T8", "T9", "T10", "validation"])).toBe("odoo_batch_implementer")
    expect(resolveAgent(registry, "IMPLEMENT", ["upgrade", "migration"])).toBe("odoo_upgrade_migrator")
